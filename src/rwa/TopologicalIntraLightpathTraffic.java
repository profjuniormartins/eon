...
public class TopologicalIntraLightpathTraffic implements EventSubscriber,Serializable {
	/** Serial version UID. */
	...
	public TopologicalIntraLightpathTraffic(int aMaxTries) {
		// System.out.println("Passei aqui tambem");
		this.maxTries = aMaxTries;
		merse = new MersenneTwister(seed);
	}

	public TopologicalIntraLightpathTraffic(int aMaxTries, long seed) {
	...
	public Object getContent() {
		...
		int numDomains = OBGPControlPlane.getNumberofDomains();
		int size = numDomains;
		...
		String source = OBGPControlPlane.getSourceNode(String
				.valueOf(chooseDomain));
		if (verbose)
			System.out.println("o SOURCE sera " + source);
		String target = OBGPControlPlane.getTargetNode(source);
		if (verbose)
